...
for i in homo_exons_data[:]:
    ...
    human_rec = float(split[18])
    chimp_rec = float(split[43])
    vervet_rec = float(split[69])
...
primary_folder = 'C:/Users/Kellen/Downloads/primate_dfe_project/raw_plots/'
...
for i in do:
    if i == 1:
        ...
        ax.set_xlim([0, 10])
        ax.set_ylim([0, 3])
        ax.set_xlabel('Human Recombination (4Ner/kb)', fontsize = 40)
